...
namespace std {
namespace detail {
template <size_t index, bool b, bool... Bs> struct first_index_true {
  static inline constexpr size_t value =
      sizeof...(Bs) == 0
          ? (b ? index : -1)
          : (b ? index : first_index_true<index + 1, Bs...>::value);
};

template <typename T, typename U> using can_construct = is_constructible<U, T>;
...
template <typename... Ts> class variant {
  ...
  friend constexpr variant_alternative_t<I, variant<Ts1...>> &
  get(variant<Ts1...> &v);
  template <size_t I, class... Ts1>
  friend constexpr variant_alternative_t<I, variant<Ts1...>> &&
  get(variant<Ts1...> &&v);
  template <size_t I, class... Ts1>
  friend constexpr const variant_alternative_t<I, variant<Ts1...>> &
  get(const variant<Ts1...> &v);
  template <size_t I, class... Ts1>
  friend constexpr const variant_alternative_t<I, variant<Ts1...>> &&
  get(const variant<Ts1...> &&v);

  template <class T, class... Ts1> friend constexpr T &get(variant<Ts1...> &v);
  ...
  template <size_t I, class... Ts1>
  friend constexpr add_pointer_t<
      const variant_alternative_t<I, variant<Ts1...>>>
  get_if(const variant<Ts1...> *pv) noexcept;

  using first_t = detail::type_pack_element<0, Ts...>;

  inline constexpr static auto align =
      detail::max_comptime<size_t, alignof(Ts)...>::value;
  inline constexpr static auto size =
      detail::max_comptime<size_t, sizeof(Ts)...>::value;

  using deleter = void (*)(char *);
  using constructor = void (*)(const char *, char *);
  using swapper = void (*)(char *, char *);

  inline constexpr static deleter destructor[] = {
      +[](char *p) { reinterpret_cast<Ts *>(p)->~Ts(); }...};
...
public:
  ...
  constexpr variant() noexcept(noexcept(first_t())) : idx(0) {
    new (data) first_t();
  }
  ...
  constexpr variant(T &&t) noexcept(noexcept(T(forward(t)))) {
    ...
    new (data) detail::type_pack_element<
        detail::first_index_true<0, is_constructible_v<Ts, T>...>::value,
        Ts...>(forward(t));
  }
  ...
  constexpr explicit variant(in_place_type_t<T>, Args &&...args) {
    ...
    new (data) detail::type_pack_element<
        detail::first_index_true<0, is_same_v<T, Ts>...>::value, Ts...>(
        forward(args...));
  }
  ...
  template <typename T, typename... Args> constexpr T &emplace(Args &&...args) {
    ...
    idx = npos;
    new (data) detail::type_pack_element<
        detail::first_index_true<0, is_same_v<Ts, T>...>::value, Ts...>(
        forward(args)...);
  ...
  template <size_t I, typename U, typename... Args>
  constexpr variant_alternative_t<I, variant> &emplace(initializer_list<U> il,
                                                       Args &&...args) {
    safe_delete();
    idx = I;
  ...
  // TODO: impl
  constexpr void
  swap(variant &rhs) noexcept(((is_nothrow_move_constructible_v<Ts> &&
