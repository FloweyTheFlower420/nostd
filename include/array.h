...
namespace std {
namespace detail {
template <typename T, size_t N> struct array_helper {
  using type = T[N];

  static constexpr T *ptr(const type &t, size_t index) noexcept {
    return &t[index];
  }
  static constexpr T &ref(const type &t, size_t index) noexcept {
    return t[index];
  }
};

template <typename T> struct array_helper<T, 0> {
  struct type {};
  static constexpr T *ptr(const type &t, size_t index) noexcept {
    return nullptr;
  }
...
template <typename T, size_t N> struct array {
  ...
  constexpr const_pointer data() const noexcept {
    return __helper::ptr(__buf, 0);
  }

  constexpr iterator begin() noexcept {
    return iterator(__helper::ptr(__buf, 0));
  }
  constexpr iterator end() noexcept {
    return iterator(__helper::ptr(__buf, N - 1));
  }
