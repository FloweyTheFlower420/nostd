...
using printf_argument_wrapper =
    variant<signed char, short, int, long, long long, unsigned char,
            unsigned short, unsigned int, unsigned long, unsigned long long,
            intmax_t, uintmax_t, ptrdiff_t, size_t, const char *, void *,
            signed char *, short *, int *, long *, long long *, intmax_t *,
            ptrdiff_t *, size_t *>;
...
public:
  template <typename... Args> fmtargs_wrapper(Args &&...args) {
    index = 0;
    size = sizeof...(args);
    fmtargs = new printf_argument_wrapper[size];
    initializer_list<printf_argument_wrapper> il{
        printf_argument_wrapper((remove_cv_t<decay_t<Args>>)args)...};
    size_t i = 0;
    for (auto e : il)
      fmtargs[i++] = e;
  }
...
template <typename T> constexpr size_t floor_log(unsigned base, T val) {
  if (val < 0)
    val = -val;
  size_t ret = 0;
  while ((val /= base))
...
template <typename T>
constexpr size_t guess_len(unsigned b, T val, unsigned flag,
                           unsigned precision) {
...
template <typename P>
void handle_special(fmtargs_wrapper &fmtargs, const char *&format,
                    printf_command &cmd, P print) {
  size_t width_len;
  const char *str;
  void *ptr;
  char buf[sizeof(void *) * 2] = {'0'};
  const char *int2char = (isupper(*format) || (cmd.flags & UPPERCASE))
                             ? "0123456789ABCDEF"
                             : "0123456789abcdef";
  switch (*format) {
  case 'c':
...
void format_int(fmtargs_wrapper &fmtargs, const printf_command &cmd,
                const char *format, size_t pc, P print) {
  char tmp[16] = {0};
  const char *int2char = (isupper(*format) || (cmd.flags & UPPERCASE))
                             ? "0123456789ABCDEF"
                             : "0123456789abcdef";
...
int sprintf(char *buffer, const char *format, Args &&...args) {
  printf_callback([&, buffer](char ch) mutable { *buffer++ = ch; }, format,
                  args...);
}
