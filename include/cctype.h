...
namespace std {
namespace detail {
template <typename T, T from, T to> constexpr bool range(T t) {
  return t >= from && t <= to;
}
} // namespace detail
...
constexpr bool iscntrl(char c) {
  return detail::range<char, 0x00, 0x1f>(c) || c == 0x7F;
}
...
constexpr bool ispunct(char c) {
  return detail::range<char, '!', '/'>(c) || detail::range<char, ';', '@'>(c) ||
         detail::range<char, '[', '`'>(c) || detail::range<char, '{', '~'>(c);
}
...
constexpr bool isxdigit(char c) {
  return isdigit(c) || detail::range<char, 'a', 'f'>(c) ||
         detail::range<char, 'A', 'F'>(c);
}
