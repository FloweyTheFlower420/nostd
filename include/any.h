...
class any {
  ...
  inline static constexpr bool
      can_inplace = is_trivially_move_constructible_v<T> &&
...
public:
  constexpr any() noexcept
      : buffer(nullptr), deleter(nullptr), copier(nullptr), mover(nullptr),
        is_inplace(false) {}
  ...
  explicit any(std::in_place_type_t<V>, std::initializer_list<U> il,
               Args &&...args)
      : is_inplace(can_inplace<std::decay_t<V>>) {
    using T = std::decay_t<V>;
    if constexpr (can_inplace<T>) {
...
template <typename T, typename... Args> std::any make_any(Args &&...args) {
