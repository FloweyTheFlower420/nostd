...
namespace std {
namespace detail {
template <typename I, typename C> class iterator_wrapper {
protected:
  I current;
...
public:
  ...
  template <typename It,
            typename = std::enable_if_t<std::is_convertible_v<It, I>>>
  iterator_wrapper(const iterator_wrapper<It, C> &i) noexcept
      : current(i.base()) {}
  ...
  iterator_wrapper operator++(int) noexcept {
    return iterator_wrapper(current++);
  }
  ...
  iterator_wrapper operator+(difference_type n) const noexcept {
    return iterator_wrapper(current + n);
  }
  iterator_wrapper operator-(difference_type n) const noexcept {
    return iterator_wrapper(current - n);
  }
...
operator==(const iterator_wrapper<It, C> &lhs,
...
[[nodiscard]] constexpr inline bool
operator==(const iterator_wrapper<ItL, C> &lhs,
...
template <typename It, typename C>
[[nodiscard]] constexpr inline typename iterator_wrapper<It, C>::difference_type
operator-(const iterator_wrapper<It, C> &lhs,
          const iterator_wrapper<It, C> &rhs) noexcept {
  return lhs.base() - rhs.base();
}
...
template <typename It, typename C>
[[nodiscard]] constexpr inline iterator_wrapper<It, C>
operator+(typename iterator_wrapper<It, C>::difference_type n,
