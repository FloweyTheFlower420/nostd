...
public:
  constexpr unique_ptr() : buf(nullptr) {}
  constexpr unique_ptr(std::nullptr_t) {}
  template <typename U> constexpr explicit unique_ptr(U p) noexcept : buf(p) {}
  template <typename U>
  constexpr unique_ptr(unique_ptr<U> &&u) noexcept : buf(u.release()) {}
  ...
  template <typename U> constexpr void reset(U ptr) noexcept {
    pointer tmp = buf;
    buf = ptr;
    if (tmp)
      delete[] tmp;
...
template <typename T> struct make_unique<T[]> {
  using array = unique_ptr<T[]>;
};
template <typename T, size_t N> struct make_unique<T[N]> {
  struct invalid_type {};
};

template <typename T>
using unique_ptr_t = typename make_unique<T>::single_object;
template <typename T> using unique_ptr_array_t = typename make_unique<T>::array;
template <typename T>
