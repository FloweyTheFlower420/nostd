...
private:
  template <typename It> friend class reverse_iterator;

  template <typename T> static constexpr T *convert_iter_to_ptr(T *p) {
    return p;
  }
  template <typename T> static constexpr pointer convert_iter_to_ptr(T t) {
    return t.operator->();
  }

public:
  constexpr reverse_iterator() noexcept(noexcept(I())) : iter() {}
  explicit constexpr reverse_iterator(iterator_type t) noexcept(noexcept(I(t)))
      : iter(t) {}
  constexpr reverse_iterator(const reverse_iterator &i) noexcept(
      noexcept(I(i.current)))
      : iter(i.current) {}
  reverse_iterator &operator=(const reverse_iterator &) = default;
  template <typename It>
  constexpr reverse_iterator(const reverse_iterator<It> &i) noexcept(
      noexcept(I(i.iter)))
      : iter(i.iter) {}

  [[nodiscard]] constexpr iterator_type base() const
      noexcept(noexcept(I(iter))) {
    return iter;
  }

  template <typename It>
  constexpr reverse_iterator &
  operator=(const reverse_iterator<It> &i) noexcept(noexcept(iter = i.iter)) {
    iter = i.iter;
    return *this;
  }
  ...
  [[nodiscard]] inline constexpr difference_type
  operator-(const reverse_iterator<ItR> &rhs) {
...
template <typename It>
[[nodiscard]] inline constexpr bool operator<(const reverse_iterator<It> &lhs,
...
template <typename ItL, typename ItR>
[[nodiscard]] inline constexpr bool
operator==(const reverse_iterator<ItL> &lhs, const reverse_iterator<ItR> &rhs) {
  return lhs.base() == rhs.base();
}
...
template <typename It>
[[nodiscard]] inline constexpr reverse_iterator<It>
